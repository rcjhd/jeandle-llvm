...
public:
  enum DeoptValueType {
    ...
    LazyObjectType = 8,
    LazyObjectFieldType = 9,
    LastType = LazyObjectFieldType + 1
  };
  DeoptValueEncoding(int I, DeoptValueType VT, int BT)
    ...
    assert((ValueTy == LocalType || ValueTy == StackType ||
            ...
            ValueTy == NarrowOopMarkerType || ValueTy == LazyObjectType ||
            ValueTy == LazyObjectFieldType) &&
           "Unsupported value type");
    assert(isValidHotspotBasicType(BasicTy) && "Unsupported basic type");
  }
  ...
  static inline DeoptValueEncoding decode(uint64_t Encode) {
    ...
    assert(isValidHotspotBasicType(BasicTypeVal) && "must be");
    return {Index, (DeoptValueType)(ValType), BasicTypeVal};
  }

  inline int index() const { return Index; }
  inline DeoptValueType valueType() const { return ValueTy; }
  inline int basicType() const { return BasicTy; }
...
inline bool isObjectDeoptEncoding(const DeoptValueEncoding &Enc) {
  int BT = Enc.basicType();
  return BT == T_OBJECT || BT == T_ARRAY;
}

inline unsigned deoptRecordEnd(unsigned Begin, unsigned End, uint64_t Width) {
  assert(Begin <= End && "invalid deopt record position");
  assert(Width <= End - Begin && "truncated deopt record");
  return Begin + static_cast<unsigned>(Width);
}

inline bool isDuplicatedDeoptBCIPair(ArrayRef<Value *> Inputs, unsigned I) {
  if (I + 1 >= Inputs.size())
    return false;
  auto *BCI0 = dyn_cast<ConstantInt>(Inputs[I]);
  auto *BCI1 = dyn_cast<ConstantInt>(Inputs[I + 1]);
  return BCI0 && BCI1 && BCI0->getType()->isIntegerTy(32) &&
         BCI1->getType()->isIntegerTy(32) &&
         BCI0->getSExtValue() == BCI1->getSExtValue();
}

inline bool isDeoptMethodMarker(Value *V) {
  auto *CI = dyn_cast<ConstantInt>(V);
  if (!CI || !CI->getType()->isIntegerTy(64))
    return false;
  return DeoptValueEncoding::decode(CI->getZExtValue()).valueType() ==
         DeoptValueEncoding::MethodType;
}

inline bool startsWithDeoptScopeHeader(ArrayRef<Value *> Inputs, unsigned I) {
  if (I >= Inputs.size())
    return false;
  if (isDeoptMethodMarker(Inputs[I]))
    I = deoptRecordEnd(I, Inputs.size(), 2);
  return isDuplicatedDeoptBCIPair(Inputs, I);
}

inline unsigned skipDeoptScopeHeader(ArrayRef<Value *> Inputs, unsigned I) {
  assert(I < Inputs.size() && "deopt scope header is missing");

  if (isDeoptMethodMarker(Inputs[I]))
    I = deoptRecordEnd(I, Inputs.size(), 2);

  assert(isDuplicatedDeoptBCIPair(Inputs, I) &&
         "deopt scope must start with a duplicated BCI pair");
  return deoptRecordEnd(I, Inputs.size(), 2);
}
